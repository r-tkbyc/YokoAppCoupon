...
### 8-2. 「各日」フラグは見えない状態なので居残らせない

`各日先着100名様` は「欄には `100`、タイトルには `※各日先着100名様`」という要件のため、`各日` の有無を `#firstCome` の **`data-each-day`** に退避している。**画面に出ない状態**で、ペーストIIFE（立てる側）と変換IIFE（読む側）を DOM 経由で繋いでいる。

- 立てる／落とすのはペースト時のみ。`各日` を含まないセルを貼れば落ちる
- 手入力で値を直したときも落とす。判定は `input` イベントの **`isTrusted`**。ペースト側が撃つのは合成イベント（`isTrusted:false`）なので、自分で立てたフラグを自分で消さずに済む。単純な「値が変わったら落とす」にすると `setFieldValue()` の dispatch で即座に消えて機能しない
