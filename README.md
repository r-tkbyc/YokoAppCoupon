...
| コンポーネント | バージョン | 更新日 |
|---|---|---|
| index.html（ツール本体） | v1.4.1 | 2026-08-21 |
...
│   ├── paste-run.ps1              … 一括ペーストの回帰テスト
│   ├── paste-corpus.js            … 上記のテストケース（Excel実出力を使用）
...
│   └── clipboard-dump.html        … クリップボードの中身を見る（ブラウザで開く）
...
**セル内の“本物の改行”は `<br>` だけ**。HTMLソース中の改行はただの空白として畳む（HTMLの空白折り畳み規則そのまま）。

Excel は長いセルを**ソース側で勝手に折り返して**出力する。
...
`textContent` をそのまま採ると、この折り返しを文字としての改行と解釈してしまい、本文の無関係な位置に改行が入る。1セルだけのコピーで症状が出なかったのは、その場合 `text/plain` の既定ペーストにまかせていてこの経路を通らないため。

| 入力 | v1.4.0 まで | v1.4.1 以降 |
|---|---|---|
| ソース側の折り返し | `"…扇子ケース\n  プレゼント"` ❌ | `"…扇子ケース プレゼント"` ✅ |
| `<br>`（Alt+Enter） | `"1階\n  化粧品売場\n\n  2階…"` ❌ | `"1階 化粧品売場\n2階…"` ✅ |
...
.\tests\paste-run.ps1  # Excel複数セルの一括ペーストの回帰テスト
