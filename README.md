...
| コンポーネント | バージョン | 更新日 |
|---|---|---|
| index.html（ツール本体） | v1.5.0 | 2026-09-11 |
...
| 先着人数 | `firstCome` | number | 数字のみを保持。変換時に全体利用回数制限へ自動反映。ペーストされたセルに `各日` が含まれていた場合は `data-each-day="1"` として記録（下記） |
...
`各日` は数値欄に文字として残せないため、`#firstCome` の **`data-each-day="1"`** に退避する。画面には出ない。

- 立てる／落とすのは**ペースト時のみ**（`各日` を含まないセルを貼れば落ちる）
- **手入力で値を直すと落ちる。** `input` イベントの `isTrusted` で切り分けており、ペースト側が撃つ合成イベント（`isTrusted:false`）では落ちない
- 読むのは変換側の `buildTitleResult()` だけ。CMSへは送らない（タイトル文字列に含まれて渡る）

⚠️ **画面に出ない状態なので、居残らせないこと。** 前のクーポンの `各日` が残ったまま次の変換をすると `※各日先着30名様` が黙って出来上がる。`data-persist` は付けておらず、リロードでも復元されない。
