...
| コンポーネント | バージョン | 更新日 |
|---|---|---|
| index.html（ツール本体） | v1.6.0 | 2026-09-11 |
...
| 出力（タイトル） | textarea `.output-title` | `transform()` 適用結果。先着人数があれば `※先着N名様`（各日フラグONなら `※各日先着N名様`）を付加 |
...
| 先着人数 | `firstCome` | number | 数字のみを保持。変換時に全体利用回数制限へ自動反映。`各日` の有無は `data-each-day="1"` に持ち、見出し横のチップ `#eachDayChip` で表示・切り替えする（下記） |
...
`各日`（後述）の有無でこの反映は変わらない。`各日先着100名様` でも全体で利用可能な回数は `100` が入る（本来は人数×日数だが、運用上そのままでよいと確認済み）。
...
**単一セルの例外（v1.5.0〜）:** 分割しない場合は原則ブラウザ既定のペーストにまかせるが、**number欄（`先着人数`）だけは肩代わりする**。`<input type="number">` は数値として読めない貼り付けをブラウザ側が丸ごと捨てるため、`先着１００名様` を1セルで貼ると何も入らないことがある。1セルでも2セルでも同じ結果になるよう、number欄は下記の抜き出し処理を通す。
...
- **number欄は「数字＋既知の語」だけを受け付ける。** 対象欄のうち number は `先着人数` のみ。詳細は下記「先着人数の文字混じりセル」。
  未知の語が残るセルは**入れずに空のまま**にし、トーストで欄名を警告する。Excelの列順とツールの欄順がずれているサインとして使える。
...
Excel側は `30` だけでなく `先着１００名様` `各日30名様` `各20名` のようにも書かれてくる。**数字部分を抜き出して欄に入れる**。

判定は文字クラスではなく**語の許可リスト**。正規化（全角数字→半角）したうえで、既知の語・数字・区切りを取り除き、**何も残らなければ通す**。

```js
const NUM_CELL_WORDS  = /各日|先着順|先着|限定|合計|各|計|名様|名|様|人|回|件|個/g;
const NUM_CELL_FILLER = /[\d,\s・、]/g;
```

| 入力セル | `#firstCome` | 各日 | タイトル末尾 |
|---|---|---|---|
| `30` | `30` | — | `※先着30名様` |
| `先着１００名様` | `100` | — | `※先着100名様` |
| `各20名` / `限定30名` / `計30名様` | `20` / `30` / `30` | — | `※先着N名様` |
| `1,000名様` | `1000` | — | `※先着1000名様` |
| `各日先着１００名様` | `100` | ✅ | `※各日先着100名様` |
| `1階 化粧品` | （空・警告） | — | — |
| `［夕張あきんど屋］じゃがバター…` | （空・警告） | — | — |
| `先着20名様30%OFF` | （空・警告） | — | — |

**語は長いものを先に並べること**（`各日`→`各`、`名様`→`名`）。JSの正規表現の選択は最長一致ではなく記述順に試すため、順序を入れ替えると `各日` が `各`＋`日` に割れて `日` が残り、正しいセルを弾く。

**`1階 化粧品` を弾く性質は緩めないこと。** 数字だけ拾うと `先着1名` という「もっともらしい誤り」が静かに通り、変換時には全体の利用回数制限＝あり／全体回数＝1 まで自動反映されてしまう。同じ理由で、貼り始める欄を1つ間違えてタイトルが先着人数へ落ちた場合も弾く必要がある。`先着20名様30%OFF` のような複数の数字を含むセルも、`%` `OFF` が残るため自動的に弾かれる（素通しすると `2030` になる）。

### 「各日」フラグの持ち回りと表示

`各日` は数値欄に文字として残せないため、`#firstCome` の **`data-each-day="1"`** に持つ。これが唯一の真実で、見出し横のチップ（`#eachDayChip`）はそれを映すだけ。

**書き手は3か所**（いずれも別スコープ）:

| 経路 | 動作 |
|---|---|
| ペースト（`setFieldValue()`） | セルに `各日` があれば立てる／無ければ落とす |
| チップのクリック | ON/OFF をトグル |
| クリア（右上バージョンバッジ） | 落とす |

書き手側に表示の同期を義務づけると必ずどれかが漏れるため、チップは **MutationObserver で `data-each-day` を監視**して追従する。クリック経路だけは即座に見た目を返すため `syncEachDayChip()` を直接も呼ぶ（冪等）。

読むのは変換側の `buildTitleResult()` だけ。CMSへは送らない（タイトル文字列に含まれて渡る）。

**表示（v1.6.0〜）:**

| 状態 | 見え方 |
|---|---|
| OFF | 透明。**先着人数パネルにマウスを乗せたときだけ**薄い枠線で浮かび上がる（平時は従来と同じ見た目） |
| ON | 青の塗りつぶしで常時表示 |

⚠️ **チップは `display:inline` のままにすること。** インライン要素の縦 padding / border は行ボックスの高さに影響しないため、見出しの高さが変わらない。`inline-block` にすると4列グリッドの中でこのパネルだけ背が高くなる。ヘッドレスで実測済み（OFF／ON／チップ削除のいずれも `panel=49.00px` `h2=14.00px` で一致）。

**手入力で人数を打ち直してもフラグは落ちない**（v1.5.0 から方針変更）。v1.5.0 では見えない状態を居残らせないために `input` の `isTrusted` で落としていたが、チップで可視化されたため不要になった。むしろ残すと「チップをONにしてから人数を打つ」が即座に無効化されて使えない。
...
加えて、`#firstCome` の **`data-each-day`（各日フラグ）も落とす**。値のクリアは `el.value = ''` のループなので data 属性には届かず、明示的に消している。残ると人数だけ消えて `各日` が居座り、次のクーポンに `※各日` が黙って付く。
